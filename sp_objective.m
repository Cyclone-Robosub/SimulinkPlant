...
imu_lin_acc = SimLog.imu_lin_acc;
...
imu_ang_vel = SimLog.imu_ang_vel;
...
dvl_vel = SimLog.dvl_vel;
vel_x = timeseries(squeeze(dvl_vel.Data(1,:,:)),dvl_vel.Time);
ref_vel_x = timeseries(squeeze(Exp.OutputData(4).Values.Data(1,:,:)),Exp.OutputData(4).Values.Time);
vel_x_error = evalRequirement(r,vel_x,ref_vel_x);
vel_y = timeseries(squeeze(dvl_vel.Data(2,:,:)),dvl_vel.Time);
ref_vel_y = timeseries(squeeze(Exp.OutputData(4).Values.Data(2,:,:)),Exp.OutputData(4).Values.Time);
vel_y_error = evalRequirement(r,vel_y,ref_vel_y);
vel_z = timeseries(squeeze(dvl_vel.Data(3,:,:)),dvl_vel.Time);
ref_vel_z = timeseries(squeeze(Exp.OutputData(4).Values.Data(3,:,:)),Exp.OutputData(4).Values.Time);
...
dvl_eul = SimLog.dvl_eul;
eul_phi = timeseries(squeeze(dvl_eul.Data(1,:,:)),dvl_eul.Time);
ref_eul_phi = timeseries(squeeze(Exp.OutputData(6).Values.Data(1,:,:)),Exp.OutputData(6).Values.Time);
eul_phi_error = evalRequirement(r,eul_phi,ref_eul_phi);
eul_psi = timeseries(squeeze(dvl_eul.Data(2,:,:)),dvl_eul.Time);
ref_eul_psi = timeseries(squeeze(Exp.OutputData(6).Values.Data(2,:,:)),Exp.OutputData(6).Values.Time);
eul_psi_error = evalRequirement(r,eul_psi,ref_eul_psi);
eul_theta = timeseries(squeeze(dvl_eul.Data(3,:,:)),dvl_eul.Time);
ref_eul_theta = timeseries(squeeze(Exp.OutputData(6).Values.Data(3,:,:)),Exp.OutputData(6).Values.Time);
eul_theta_error = evalRequirement(r,eul_theta,ref_eul_theta);


dvl_pos = SimLog.dvl_pos;
pos_x = timeseries(squeeze(dvl_pos.Data(1,:,:)),dvl_pos.Time);
ref_pos_x = timeseries(squeeze(Exp.OutputData(7).Values.Data(1,:,:)),Exp.OutputData(7).Values.Time);
pos_x_error = evalRequirement(r,pos_x,ref_pos_x);
pos_y = timeseries(squeeze(dvl_pos.Data(2,:,:)),dvl_pos.Time);
ref_pos_y = timeseries(squeeze(Exp.OutputData(7).Values.Data(2,:,:)),Exp.OutputData(7).Values.Time);
pos_y_error = evalRequirement(r,pos_y,ref_pos_y);
pos_z = timeseries(squeeze(dvl_pos.Data(3,:,:)),dvl_pos.Time);
ref_pos_z = timeseries(squeeze(Exp.OutputData(7).Values.Data(3,:,:)),Exp.OutputData(7).Values.Time);
