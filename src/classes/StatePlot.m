classdef StatePlot
    %{
    Class used by setupPlots to create high quality plots
    with a large amount of code reuse.
    %}
    properties
        %set by constructor
        name %1x1 string
        fields %1xN string (which fields of results we need)
        subfields %1xN string (which sub-fields of the structure we need
        structures = {} %1xN structures corresponding with each field
        timeseries = {} %1xN timeseries corresponding with each subfield
    ...
    methods
        function obj = StatePlot(name, fields, subfields, layout, signal_map, v_labels, h_labels, subtitles, supertitle, legends)
            %constructor assigns class members and validates input
            obj.name = name;
            obj.fields = fields;
            obj.subfields = subfields;
            obj.layout = layout;
            ...
            try
                ...
                for k = 1:length(obj.fields) %unpack the timeseries from the results structure
                    if(ismember(obj.fields(k),results.who))
                        obj.structures{end+1} = results.(obj.fields(k));
                    end
                end
                
                for j = 1:length(obj.structures)
                    for k = 1:length(obj.subfields) %unpack the time and signals from the timeseries
                        signals_k = squeeze(obj.structures{j}.(obj.subfields(k)).Data);
                        signals_k = enforceTallSkinny(signals_k);
                        t_k = obj.structures{j}.(obj.subfields(k)).Time;

                        [~,cols] = size(signals_k);
                        for m = 1:cols 
                            obj.signals(:,end+1) = signals_k(:,m); 
        ...
        function plot(obj, results)
            ...
            plot(foo) function to input foo of type TimePlot.
            %}
