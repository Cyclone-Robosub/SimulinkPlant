%Monte carlo init generator rough draft
...
xi_0 = randomIC(-10,10,10);
yi_0 = randomIC(-10,10,10);
zi_0 = randomIC(-10,10,10);
Ri_0 = [xi_0; yi_0; zi_0];

%initial intertial velocity
ui_0 = randomIC(-1,1,10);
vi_0 = randomIC(-1,1,10);
wi_0 = randomIC(-1,1,10);
dRi_0 = [ui_0; vi_0; wi_0];

%initial euler angles
phi_0 = randomIC(-pi,pi,10);
theta_0 = randomIC(-pi/2,pi/2,10);
psi_0 = randomIC(-pi,pi,10);
Eul_0 = [phi_0; theta_0; psi_0];

%other attitude representations
for k = 1:10
    Cib_0_k = eul2rotm([psi_0(k),theta_0(k),phi_0(k)],'ZYX');
    q_0_k = rotm2quat(Cib_0_k); 
    %reorder because the scalar should be at the end for calcs in the sim
    q_0_k = [q_0_k(2); q_0_k(3); q_0_k(4); q_0_k(1)];
    q_0(:,k) = q_0_k;
end
...
%pack initial state
for k = 1:10
    X0 = [Ri_0(:,k);q_0(:,k);dRi_0(:,k);wb_0(:,k)];
    ...
    saveAllOutputs(results,prj_paths_list.user_data_path);
