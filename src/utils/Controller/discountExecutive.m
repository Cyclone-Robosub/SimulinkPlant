...
status is SUCCESS or FAIL - do nothing if the status is RUNNING Check the
status of the timeout - advance to next if the timeout has elapsed - do
nothing otherwise
...
are "SUCC", "FAIL", and "RUNN" for success, failure, and running. mission
(Nx1 struct) - the vector of commands output by importMission.m. Each
command in the mission follows the template defined by setup_cmd_bus.m
