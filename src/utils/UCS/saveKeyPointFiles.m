...
for i = 1:numPerspectives
    writematrix(results.keyPointsImageList(i + startDelay_KP/2,:), fullfile(save_file_path,"CSV_Files/frame_" + i + ".csv"));
    camImage = results.camL_Feed(:, :, :, i + startDelay_KP/2 + 1);
    imwrite(camImage, fullfile(save_file_path,"Pictures/frame_" + i + ".jpg"));
    
    lkp = results.keyPointsImageList(i + startDelay_KP/2, :);
    ...
    imwrite(drawnImage, fullfile(save_file_path,"Pictures/drawn_frame_" + i + ".jpg"));
end
